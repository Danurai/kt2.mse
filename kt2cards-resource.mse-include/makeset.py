...
def writeitem( file, item, faction):
	...
	if 'text' in item:
		file.write('\trule: ' + convertmarkdown(item['text']) + '\n')
	if 'ability' in item:
		file.write('\trule:\n\t\t{}This operative gains the following ability for the battle:\n\t\t\n\t\t<b>{}:</b> {}\n'.format(oprestriction, item['name'],convertmarkdown(item['ability']['text'])))
		file.write('\teffect:\tEffect\n')
	if 'weapon' in item:
		...
		if 'sr' in item['weapon']: file.write('\twpn_sr: {}\n'.format(convertmarkdown(', '.join(item['weapon']['sr']))))
		if 'i' in item['weapon']: file.write('\twpn_i: {}\n'.format(convertmarkdown(', '.join(item['weapon']['i']))))
	if 'action' in item:
...
ktdata = json.load(open('./kt2cards-resource.mse-include/kt2data.json'))
